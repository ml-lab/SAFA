...
class OcclusionAwareGenerator(nn.Module):
    ...
    def __init__(self, num_channels, num_kp, block_expansion, max_features, num_down_blocks,
        ...
        for i in range(num_down_blocks):
            ...
            # up_blocks.append(GADEUpBlock2d(in_features*2, out_features, kernel_size=(3, 3), padding=(1, 1)))
        ...
        self.fc_1 = nn.Linear(159, in_features)
        self.fc_2 = nn.Linear(159, in_features)
    ...
    def forward(self, source_image, kp_driving=None, kp_source=None, render_ops=None, 
        ...
        code = driving_features['code']
        # 1. normalize each part of the code separately
        code[:, 0:100] = code[:, 0:100] * 0.02
        code[:, 100:150] = code[:, 100:150] * 0.03
        code[:, 156] = code[:, 156] * 0.25

        # 2. geometric feature injection
        ...
        for i in range(len(self.up_blocks)):
            deformed_source = self.deform_input(source_features[len(self.down_blocks)-i], deformation)
            out = torch.cat([deformed_source, out], dim=1)
            out = self.up_blocks[i](out)
            #out = self.up_blocks[i](out, code)   # in case of GADEUpBlock2d
        deformed_source = self.deform_input(source_features[0], deformation)
        out = torch.cat([deformed_source, out], dim=1)
